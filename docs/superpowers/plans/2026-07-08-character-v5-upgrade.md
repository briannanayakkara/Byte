...
With Byte in a calm mood (e.g. `neutral`), move the cursor slowly toward him from a moderate distance — confirm his head turns to track it first, then once close enough he walks (or runs, if the gap is large) over and settles into a little hop-hop, continuing to track the cursor. Move the cursor away or hold it still for a few seconds — confirm he disengages and drifts back toward center. Confirm sending a chat message or waiting for an idle-move tick correctly interrupts an in-progress chase (per the existing `follow.on = false` reset in `setMood`, Task 4 Step 6). **Correction from an earlier draft of this step:** cursor-follow does *not* interrupt an in-progress `skate`/`playball`/`jam` routine — `followOk` deliberately excludes those three moods (confirmed against the reference file's own `FOLLOW_OK` definition), so proximity alone can never engage follow while one is playing; a Play routine only ends via an explicit `Byte.set()` call (e.g. the idle-move timer's next regular tick) or by looping indefinitely otherwise. Confirm this is in fact the observed behavior — cursor proximity during a Play routine should do nothing. Run `window.Byte.interactive(false)` in the console — confirm hovering/clicking no longer does anything; `window.Byte.interactive(true)` restores it.
